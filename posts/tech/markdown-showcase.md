...
- 也可以写 `.md` 让 VitePress 解析，路径相对当前文件：
  [笔记那篇](../notes/note-taking.md)
- 当前页锚点：[跳到代码块](#代码块)
- 带标题的链接：[VitePress 官网](https://vitepress.dev "VitePress")
- 裸链接会自动识别：https://vitepress.dev

::: tip 相对链接可以写 .md
`../notes/note-taking.md` 会被解析成构建后的 `/posts/notes/note-taking.html`，
...
表格里的 `行内代码`、[链接](../notes/note-taking.md) 与 <kbd>键位</kbd> 都能正常用。
...
本站不引入位图，图片的用法和别的 VitePress 站点一致：
