 ...
 *
 * 构建产物的真实层叠顺序由 scripts/check-rendered-parity.mjs 用无头浏览器
 * 实测（比读源码可靠得多），这里只挡住源码层面的退化。
 *
...
const stylesDir = join(root, 'packages', 'vitepress-theme-fluxixix', 'src', 'styles')
...
const siteEntry = join(root, '.vitepress', 'theme', 'index.ts')
const siteEntrySrc = readFileSync(siteEntry, 'utf8')
const importsPackageEntry = /from\s+['"]vitepress-theme-fluxixix\/theme['"]/.test(siteEntrySrc)
const importsCssDirectly = /vitepress-theme-fluxixix\/styles\/index\.css/.test(siteEntrySrc)

if (!importsPackageEntry && !importsCssDirectly) {
  errors.push(
    '.vitepress/theme/index.ts 没有引主题样式：站点页面上不会有主题样式'
  )
}
 ...
 * 而且这种情况不报任何错，只能靠断言挡。
 */
const distAssets = join(root, '.vitepress', 'dist', 'assets')
if (existsSync(distAssets)) {
  const bundles = readdirSync(distAssets).filter((n) => n.endsWith('.css'))
  for (const bundle of bundles) {
    ...
    if (themeAt < vpAt) {
      errors.push(
        `.vitepress/dist/assets/${bundle}：主题令牌排在第 ${themeAt} 字节、` +
          `VitePress 默认变量在第 ${vpAt} 字节 —— 主题在前就会被默认主题反压。` +
          '检查 .vitepress/theme/index.ts 里主题样式与主题本体的引入顺序'
      )
    }
    if (/@layer\s+fluxixix-/.test(css)) {
      errors.push(
        `.vitepress/dist/assets/${bundle}：出现 @layer fluxixix-*，` +
...
console.log(
  `✓ 层约定检查通过：${allFiles.length} 个样式文件都未分层；总表覆盖全部文件；` +
    (importsPackageEntry
      ? '站点经 vitepress-theme-fluxixix/theme 引入（顺序由包保证）'
      : '站点直接引入样式总表') +
    (existsSync(distAssets) ? '；产物顺序正确（主题排在默认主题之后）' : '')
)
